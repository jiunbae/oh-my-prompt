...
### Performance
- Capture no longer rewrites the whole database several times per turn. sql.js serialises the entire file on every save outside a transaction, and the `tool_invocations` insert loop did one save *per tool call* — a single agentic turn could write hundreds of gigabytes and hold the ingest lock for minutes. The tool loop, and the prompt row together with its FTS mirror, are now one transaction each: 42 full-file rewrites for a 40-tool turn became 1, and a 200-tool capture went from ~9 minutes to 18.6s on a 285MB database
...
- `omp sync` no longer loads the entire prompts table into memory when it has no checkpoint (a fresh device, or the cursor reset after a backfill). Rows are fetched 2000 at a time and the lock is released between pages

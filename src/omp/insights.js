const signalDefinitions = [
  {
    ...
    test: (text) => /\b(goal|objective|task|you are to|your task|i need|i want)\b/.test(text),
  },
  {
    id: "context",
    label: "Context",
  ...
  },
  {
    ...
    test: (text) =>
      /\b(constraints|requirements|must|should|avoid|do not|don't|only|limit)\b/.test(text),
  },
  {
    id: "output",
    label: "Output format",
    ...
    test: (text) =>
      /\b(output format|format|respond with|return|json|markdown|table|bullet|list)\b/.test(text),
  },
  {
    id: "examples",
    label: "Examples",
...
const coreSignalIds = ["goal", "context", "constraints", "output"];
...
function analyzePrompt(rawText) {
  ...
  if (missing.has("goal")) suggestions.push("State the exact outcome or task in one sentence.");
  if (missing.has("context")) suggestions.push("Add brief context so the agent can disambiguate.");
  if (missing.has("constraints")) suggestions.push("List constraints, limits, and things to avoid.");
  if (missing.has("output")) suggestions.push("Specify the output format (bullets, JSON, table, etc.).");
  if (missing.has("examples")) suggestions.push("Include a short example of the desired output.");
  if (words < 20) suggestions.push("Add a bit more detail so the agent has enough context.");
  if (words > 400) suggestions.push("Consider splitting this into smaller steps or sections.");
...
function summarizePromptReviews(reviews) {

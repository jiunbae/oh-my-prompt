...
function getServeConfig(config, options = {}) {
  const image = config.serve?.image || "ghcr.io/jiunbae/oh-my-prompt:latest";
  const port = Number.parseInt(options.port, 10) || config.serve?.port || 3000;
  const requestedHost = options.host || config.serve?.host || "127.0.0.1";
  if (requestedHost !== "127.0.0.1" && requestedHost !== "0.0.0.0") {
    throw new Error("Serve host must be 127.0.0.1 or 0.0.0.0");
  }
  const adminEmail = config.serve?.adminEmail || config.user?.email || "";
  return { image, port, host: requestedHost, adminEmail };
}
...
services:
  ...
  app:
    ...
    restart: unless-stopped
    ports:
      - "${serveConfig.host}:${serveConfig.port}:3000"
    environment:
      DATABASE_URL: postgresql://prompt:omp-local@db:5432/prompts
      REDIS_URL: redis://redis:6379
      SESSION_SECRET: "${secret}"
      # omp serve publishes plain HTTP. Deploy behind TLS and manage your own
      # compose file before enabling Secure cookies.
      COOKIE_SECURE: "false"
...
`;
  ensureDir(SERVE_DIR);
  fs.writeFileSync(COMPOSE_FILE, yaml, { mode: 0o600 });
  fs.chmodSync(COMPOSE_FILE, 0o600);
  return COMPOSE_FILE;
}
...
async function startServer(config, options = {}) {
  checkDocker();
  const serveConfig = getServeConfig(config, options);
